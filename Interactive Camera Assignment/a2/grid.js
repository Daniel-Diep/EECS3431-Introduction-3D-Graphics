...
function drawGrid(plane, range)
{
	...
	case 'XY':
		...
		points.push(vec4(0, range, 1));
		points.push(vec4(0, -range, 1));
		points.push(vec4(range, 0, 1));
		points.push(vec4(-range, 0, 1));
		gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(points));
		gl.drawArrays(gl.LINES, 0, points.length);
		break;
	case 'XZ':
		...
		points = [];
		gl.uniform4fv(objectColor, black);
		points.push(vec4(0, 1, range));
		points.push(vec4(0, 1, -range));
		points.push(vec4(range, 1, 0));
		points.push(vec4(-range, 1, 0));
		gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(points));
		gl.drawArrays(gl.LINES, 0, points.length);
		break;
	case 'YZ':
		...
		points = [];
		gl.uniform4fv(objectColor, black);
		points.push(vec4(1, 0, range));
		points.push(vec4(1, 0, -range));
		points.push(vec4(1, range, 0));
		points.push(vec4(1, -range, 0));
		gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(points));
		gl.drawArrays(gl.LINES, 0, points.length);
		break;
